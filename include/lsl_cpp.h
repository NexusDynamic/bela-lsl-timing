 ...
 *
 * To use this library you need to link to the shared library (lsl) that comes
 * with this header. Under Visual Studio the library is linked in automatically.
 */
 ...
 *
 * Note that significantly larger numbers can cause the timeout to be invalid on
 * some operating systems (e.g., 32-bit UNIX).
 */
...
enum channel_format_t {
    ...
    accurately.*/
    cf_float32 = 1,
    /// For universal numeric data as long as permitted by network & disk
    /// budget. The largest representable integer is 53-bit.
    cf_double64 = 2,
    ...
    cf_int32 = 4,
    /// For very high rate signals (40Khz+) or consumer-grade audio (for
...
enum processing_options_t {
    ...
    post_none = 0,
    /// Perform automatic clock synchronization; equivalent to manually adding
    /// the time_correction()
    /// value to the received time stamps.
    post_clocksync = 1,
    /// Remove jitter from time stamps. This will apply a smoothing algorithm to
    /// the received time
    /// stamps; the smoothing needs to see a minimum number of samples (30-120
    /// seconds worst-case)
 ...
 * giving a a debugging person a good idea which exact library version is used.
 */
 ...
 * USB transmission delays), it can be used as an offset to local_clock() to
 * obtain a better estimate of when a sample was actually captured. See
 * stream_outlet::push_sample() for a use case.
 */
 ...
 *  c) optional meta-data about the stream content (channel labels, measurement
 * units, etc.)
 *
 * Whenever a program wants to provide a new stream on the lab network it will
 * typically first create a stream_info to describe its properties and then
 * construct a stream_outlet with it to create the stream on the network.
 * Recipients who discover the outlet can query the stream_info; it is also
...
class stream_info {
  public:
     ...
     *
     * Core stream information is specified here. Any remaining meta-data can be
     * added later.
     ...
     * @param type Content type of the stream. Please see
     * https://github.com/sccn/xdf/wiki/Meta-Data (or web search for: XDF
     * meta-data) for pre-defined content-type names, but you can also make up
     * your own. The content type is the preferred way to find streams (as
     * opposed to searching by name).
     * @param channel_count Number of channels per sample. This stays constant
     * for the lifetime of the stream.
     * @param nominal_srate The sampling rate (in Hz) as advertised by the data
     * source, if regular (otherwise set to IRREGULAR_RATE).
     * @param channel_format Format/type of each channel. If your channels have
     * different formats, consider supplying multiple streams or use the largest
     * type that can hold them all (such as cf_double64).
     * @param source_id Unique identifier of the device or source of the data,
     * if available (such as the serial number). This is critical for system
     * robustness since it allows recipients to recover from failure even after
     * the serving app, device or computer crashes (just by finding a stream
     * with the same source id on the network again). Therefore, it is highly
    ...
    stream_info(const std::string& name, const std::string& type,
        ...
        : obj(lsl_create_streaminfo(
                  (name.c_str()), (type.c_str()), channel_count, nominal_srate,
                  (lsl_channel_format_t)channel_format, (source_id.c_str())),
              &lsl_destroy_streaminfo) {
        if (obj == nullptr)
            throw std::invalid_argument(lsl_last_error());
    }
     ...
     * refers to the type of device or product series that is generating the
     * data of the stream. If the source is an application, the name may be a
     * more generic or specific identifier. Multiple streams with the same name
     * can coexist, though potentially at the cost of ambiguity (for the
     * recording app or experimenter).
     */
     ...
     *
     * Note that no data will be lost even if this sampling rate is incorrect or
     * if a device has temporary hiccups, since all samples will be recorded
     * anyway (except for those dropped by the device itself). However, when the
     * recording is imported into an application, a good importer may correct
     ...
     * recording program) can re-acquire a stream automatically once it is back
     * online.
     */
     ...
     *
     * This is a unique identifier of the stream outlet, and is guaranteed to be
     ...
     * recording activitites on the same sub-network (e.g., in multiple
     * experiment areas) from seeing each other's streams (assigned via a
     * configuration file by the experimenter, see Network Connectivity in the
     * LSL wiki).
     */
     ...
     * here. See code examples on the LSL wiki. Other information, such as
     * amplifier settings, measurement units if deviating from defaults, setup
     * information, subject information, etc., can be specified here, as well.
     * Meta-data recommendations follow the XDF file format project
     ...
     * recommendations exist, please try to lay out your meta-data in agreement
     * with these recommendations for compatibility with other applications.
     */
    xml_element desc();
     ...
     * streaminfo class, including:
     *
     *   - the core elements `<name>`, `<type>`, `<channel_count`,
     * `<nominal_srate>`,
     *   `<channel_format>`, `<source_id>`
     *   - the misc elements `<version>`, `<created_at>`, `<uid>`,
     * `<session_id>`,
...
/** A stream outlet.
 * Outlets are used to make streaming data (and the meta-data) available on the
 * lab network.
 */
class stream_outlet {
  public:
    /** Establish a new stream outlet. This makes the stream discoverable.
     * @param info The stream information to use for creating this stream. Stays
     * constant over the lifetime of the outlet.
     * @param chunk_size Optionally the desired chunk granularity (in samples)
     * for transmission. If unspecified, each push operation yields one chunk.
     * Inlets can override this setting.
     * @param max_buffered Optionally the maximum amount of data to buffer (in
     * seconds if there is a nominal sampling rate, otherwise x100 in samples).
     * The default is 6 minutes of data.
     */
    stream_outlet(const stream_info& info, int32_t chunk_size = 0,
                  int32_t max_buffered = 360,
                  lsl_transport_options_t flags = transp_default)
        : channel_count(info.channel_count()),
          sample_rate(info.nominal_srate()),
          obj(lsl_create_outlet_ex(info.handle().get(), chunk_size,
                                   max_buffered, flags),
              &lsl_destroy_outlet) {}
     ...
     * @param timestamp Optionally the capture time of the sample, in agreement
     * with lsl::local_clock(); if omitted, the current time is used.
     * @param pushthrough Whether to push the sample through to the receivers
     * instead of buffering it with subsequent samples. Note that the
     * chunk_size, if specified at outlet construction, takes precedence over
     * the pushthrough flag.
     */
    template <class T, int32_t N>
    void push_sample(const T data[N], double timestamp = 0.0,
     ...
     * Each entry in the vector corresponds to one channel. The function handles
     * type checking & conversion.
     * @param data A vector of values to push (one for each channel).
     * @param timestamp Optionally the capture time of the sample, in agreement
     * with local_clock(); if omitted, the current time is used.
     * @param pushthrough Whether to push the sample through to the receivers
     * instead of buffering it with subsequent samples. Note that the
     * chunk_size, if specified at outlet construction, takes precedence over
     * the pushthrough flag.
     */
    template <typename T>
    void push_sample(const std::vector<T>& data, double timestamp = 0.0,
                     bool pushthrough = true) {
     ...
     * This is a lower-level function for cases where data is available in some
     * buffer. Handles type checking & conversion.
     * @param data A pointer to values to push. The number of values pointed to
     * must not be less than the number of channels in the sample.
     * @param timestamp Optionally the capture time of the sample, in agreement
     * with local_clock(); if omitted, the current time is used.
     * @param pushthrough Whether to push the sample through to the receivers
     * instead of buffering it with subsequent samples. Note that the
     ...
     * the pushthrough flag.
     */
    void push_sample(const float* data, double timestamp = 0.0,
                     bool pushthrough = true) {
        lsl_push_sample_ftp(obj.get(), (data), timestamp, pushthrough);
    }
    void push_sample(const double* data, double timestamp = 0.0,
                     bool pushthrough = true) {
        lsl_push_sample_dtp(obj.get(), (data), timestamp, pushthrough);
    }
    void push_sample(const int64_t* data, double timestamp = 0.0,
                     bool pushthrough = true) {
        lsl_push_sample_ltp(obj.get(), (data), timestamp, pushthrough);
    }
    void push_sample(const int32_t* data, double timestamp = 0.0,
                     bool pushthrough = true) {
        lsl_push_sample_itp(obj.get(), (data), timestamp, pushthrough);
    }
    void push_sample(const int16_t* data, double timestamp = 0.0,
                     bool pushthrough = true) {
        lsl_push_sample_stp(obj.get(), (data), timestamp, pushthrough);
    }
    void push_sample(const char* data, double timestamp = 0.0,
                     bool pushthrough = true) {
        lsl_push_sample_ctp(obj.get(), (data), timestamp, pushthrough);
    }
    void push_sample(const std::string* data, double timestamp = 0.0,
                     bool pushthrough = true) {
        ...
        lsl_push_sample_buftp(obj.get(), pointers.data(), lengths.data(),
                              timestamp, pushthrough);
    }

    /** Push a packed C struct (of numeric data) as one sample into the outlet
     * (search for
     * [`#``pragma pack`](https://stackoverflow.com/a/3318475/73299) for
     * information on packing structs appropriately).<br> Overall size checking
     * but no type checking or conversion are done.<br> Can not be used
     * forvariable-size / string-formatted data.
     ...
     */
    template <class T>
    void push_numeric_struct(const T& sample, double timestamp = 0.0,
                             bool pushthrough = true) {
        if (info().sample_bytes() != sizeof(T))
            throw std::runtime_error("Provided object size does not match the "
                                     "stream's sample size.");
        push_numeric_raw((void*)&sample, timestamp, pushthrough);
    }

    /** Push a pointer to raw numeric data as one sample into the outlet.
     * This is the lowest-level function; performs no checking whatsoever.
     * Cannot be used for variable-size / string-formatted channels.
     ...
     * the pushthrough flag.
     */
    void push_numeric_raw(const void* sample, double timestamp = 0.0,
                          bool pushthrough = true) {
     ...
     * @param samples A vector of samples in some supported format (each sample
     * can be a data pointer, data array, or std vector of data).
     * @param timestamp Optionally the capture time of the most recent sample,
     * in agreement with local_clock(); if omitted, the current time is used.
     * The time stamps of other samples are automatically derived according to
     * the sampling rate of the stream.
     * @param pushthrough Whether to push the chunk through to the receivers
     * instead of buffering it with subsequent samples. Note that the
     * chunk_size, if specified at outlet construction, takes precedence over
     * the pushthrough flag.
     */
    template <class T>
    void push_chunk(const std::vector<T>& samples, double timestamp = 0.0,
                    bool pushthrough = true) {
        if (!samples.empty()) {
            if (timestamp == 0.0)
            ...
            push_sample(samples[0], timestamp,
                        pushthrough && samples.size() == 1);
            for (std::size_t k = 1; k < samples.size(); k++)
                push_sample(samples[k], DEDUCED_TIMESTAMP,
                            pushthrough && k == samples.size() - 1);
        }
    }

    /** Push a chunk of samples (batched into an STL vector) into the outlet.
     * Allows to specify a separate time stamp for each sample (for
     * irregular-rate streams).
     * @param samples A vector of samples in some supported format (each sample
     * can be a data pointer, data array, or std vector of data).
     * @param timestamps A vector of capture times for each sample, in agreement
     * with local_clock().
     * @param pushthrough Whether to push the chunk through to the receivers
     * instead of buffering it with subsequent samples. Note that the
     * chunk_size, if specified at outlet construction, takes precedence over
    ...
    /** Push a chunk of numeric data as C-style structs (batched into an STL
     * vector) into the outlet. This performs some size checking but no type
     * checking. Can not be used for variable-size / string-formatted data.
    ...
    void push_chunk_numeric_structs(const std::vector<T>& samples,
        ...
        if (!samples.empty()) {
            ...
            if (sample_rate != IRREGULAR_RATE)
                timestamp = timestamp - (samples.size() - 1) / sample_rate;
            push_numeric_struct(samples[0], timestamp,
                                pushthrough && samples.size() == 1);
            for (std::size_t k = 1; k < samples.size(); k++)
                push_numeric_struct(samples[k], DEDUCED_TIMESTAMP,
                                    pushthrough && k == samples.size() - 1);
        }
    }

    /** Push a chunk of numeric data from C-style structs (batched into an STL
     * vector), into the outlet. This performs some size checking but no type
     * checking. Can not be used for variable-size / string-formatted data.
     * @param samples A vector of samples, as C structs.
     * @param timestamps A vector of capture times for each sample, in agreement
     ...
     * @name Push functions
     * @param buffer A buffer of channel values holding the data for zero or
     * more successive samples to send.
     * @param timestamp Optionally the capture time of the most recent sample,
     * in agreement with local_clock(); if omitted, the current time is used.
     * The time stamps of other samples are automatically derived according to
     * the sampling rate of the stream.
    ...
    void push_chunk_multiplexed(const std::vector<T>& buffer,
                                ...
                                bool pushthrough = true) {
        if (!buffer.empty())
            push_chunk_multiplexed(buffer.data(),
                                   static_cast<unsigned long>(buffer.size()),
                                   timestamp, pushthrough);
    }

    /** Push a chunk of multiplexed data into the outlet. One timestamp per
     * sample is provided. Allows to specify a separate time stamp for each
     * sample (for irregular-rate streams).
     * @param buffer A buffer of channel values holding the data for zero or
     * more successive samples to send.
     * @param timestamps A buffer of timestamp values holding time stamps for
    ...
    void push_chunk_multiplexed(const std::vector<T>& buffer,
        ...
        if (!buffer.empty() && !timestamps.empty())
            push_chunk_multiplexed(buffer.data(),
                                   static_cast<unsigned long>(buffer.size()),
                                   timestamps.data(), pushthrough);
    }

    /** Push a chunk of multiplexed samples into the outlet. Single timestamp
     * provided.
     * @warning The provided buffer size is measured in channel values (e.g.,
     * floats), not samples.
     * @param buffer A buffer of channel values holding the data for zero or
     * more successive samples to send.
     * @param buffer_elements The number of channel values (of type T) in the
     * buffer. Must be a multiple of the channel count.
     * @param timestamp Optionally the capture time of the most recent sample,
     * in agreement with local_clock(); if omitted, the current time is used.
     * The time stamps of other samples are automatically derived based on the
     * sampling rate of the stream.
     * @param pushthrough Whether to push the chunk through to the receivers
     * instead of buffering it with subsequent samples. Note that the
     * stream_outlet() constructur parameter @p chunk_size, if specified at
     * outlet construction, takes precedence over the pushthrough flag.
     */
    void push_chunk_multiplexed(const float* buffer,
                                std::size_t buffer_elements,
                                double timestamp = 0.0,
                                bool pushthrough = true) {
        lsl_push_chunk_ftp(obj.get(), buffer,
                           static_cast<unsigned long>(buffer_elements),
                           timestamp, pushthrough);
    }
    void push_chunk_multiplexed(const double* buffer,
                                std::size_t buffer_elements,
                                double timestamp = 0.0,
                                bool pushthrough = true) {
        lsl_push_chunk_dtp(obj.get(), buffer,
                           static_cast<unsigned long>(buffer_elements),
                           timestamp, pushthrough);
    }
    void push_chunk_multiplexed(const int64_t* buffer,
                                std::size_t buffer_elements,
                                double timestamp = 0.0,
                                bool pushthrough = true) {
        lsl_push_chunk_ltp(obj.get(), buffer,
                           static_cast<unsigned long>(buffer_elements),
                           timestamp, pushthrough);
    }
    void push_chunk_multiplexed(const int32_t* buffer,
                                std::size_t buffer_elements,
                                double timestamp = 0.0,
                                bool pushthrough = true) {
        lsl_push_chunk_itp(obj.get(), buffer,
                           static_cast<unsigned long>(buffer_elements),
                           timestamp, pushthrough);
    }
    void push_chunk_multiplexed(const int16_t* buffer,
                                std::size_t buffer_elements,
                                double timestamp = 0.0,
                                bool pushthrough = true) {
        lsl_push_chunk_stp(obj.get(), buffer,
                           static_cast<unsigned long>(buffer_elements),
                           timestamp, pushthrough);
    }
    void push_chunk_multiplexed(const char* buffer, std::size_t buffer_elements,
                                double timestamp = 0.0,
                                bool pushthrough = true) {
        lsl_push_chunk_ctp(obj.get(), buffer,
                           static_cast<unsigned long>(buffer_elements),
                           timestamp, pushthrough);
    }
    void push_chunk_multiplexed(const std::string* buffer,
                                std::size_t buffer_elements,
                                double timestamp = 0.0,
                                bool pushthrough = true) {
        if (buffer_elements) {
    ...
    /** Push a chunk of multiplexed samples into the outlet. One timestamp per
     * sample is provided.
     * @warning Note that the provided buffer size is measured in channel values
     * (e.g., floats) rather than in samples.
     * @param data_buffer A buffer of channel values holding the data for zero
     * or more successive samples to send.
     * @param timestamp_buffer A buffer of timestamp values holding time stamps
     * for each sample in the data buffer.
     * @param data_buffer_elements The number of data values (of type T) in the
     * data buffer. Must be a multiple of the channel count.
     * @param pushthrough Whether to push the chunk through to the receivers
     * instead of buffering it with subsequent samples. Note that the
     * chunk_size, if specified at outlet construction, takes precedence over
     * the pushthrough flag.
     */
    void push_chunk_multiplexed(const float* data_buffer,
                                const double* timestamp_buffer,
                                std::size_t data_buffer_elements,
                                bool pushthrough = true) {
        lsl_push_chunk_ftnp(obj.get(), data_buffer,
                            static_cast<unsigned long>(data_buffer_elements),
                            (timestamp_buffer), pushthrough);
    }
    void push_chunk_multiplexed(const double* data_buffer,
                                const double* timestamp_buffer,
                                std::size_t data_buffer_elements,
                                bool pushthrough = true) {
        lsl_push_chunk_dtnp(obj.get(), data_buffer,
                            static_cast<unsigned long>(data_buffer_elements),
                            (timestamp_buffer), pushthrough);
    }
    void push_chunk_multiplexed(const int64_t* data_buffer,
                                const double* timestamp_buffer,
                                std::size_t data_buffer_elements,
                                bool pushthrough = true) {
        lsl_push_chunk_ltnp(obj.get(), data_buffer,
                            static_cast<unsigned long>(data_buffer_elements),
                            (timestamp_buffer), pushthrough);
    }
    void push_chunk_multiplexed(const int32_t* data_buffer,
                                const double* timestamp_buffer,
                                std::size_t data_buffer_elements,
                                bool pushthrough = true) {
        lsl_push_chunk_itnp(obj.get(), data_buffer,
                            static_cast<unsigned long>(data_buffer_elements),
                            (timestamp_buffer), pushthrough);
    }
    void push_chunk_multiplexed(const int16_t* data_buffer,
                                const double* timestamp_buffer,
                                std::size_t data_buffer_elements,
                                bool pushthrough = true) {
        lsl_push_chunk_stnp(obj.get(), data_buffer,
                            static_cast<unsigned long>(data_buffer_elements),
                            (timestamp_buffer), pushthrough);
    }
    void push_chunk_multiplexed(const char* data_buffer,
                                const double* timestamp_buffer,
                                std::size_t data_buffer_elements,
                                bool pushthrough = true) {
        lsl_push_chunk_ctnp(obj.get(), data_buffer,
                            static_cast<unsigned long>(data_buffer_elements),
                            (timestamp_buffer), pushthrough);
    }

    void push_chunk_multiplexed(const std::string* data_buffer,
                                const double* timestamp_buffer,
                                std::size_t data_buffer_elements,
                                bool pushthrough = true) {
        if (data_buffer_elements) {
            ...
            lsl_push_chunk_buftnp(
                obj.get(), pointers.data(), lengths.data(),
                static_cast<unsigned long>(data_buffer_elements),
                timestamp_buffer, pushthrough);
        }
    }
     ...
     * While it does not hurt, there is technically no reason to push samples if
    ...
    bool wait_for_consumers(double timeout) {
        return lsl_wait_for_consumers(obj.get(), timeout) != 0;
    }

    /** Retrieve the stream info provided by this outlet.
     * This is what was used to create the stream (and also has the Additional
     * Network Information fields assigned).
     */
    stream_info info() const { return stream_info(lsl_get_info(obj.get())); }

    /// Return a shared pointer to pass to C-API functions that aren't wrapped
    /// yet
    ///
    /// Example: @code lsl_push_chunk_buft(outlet.handle().get(), data, …);
    /// @endcode
    std::shared_ptr<lsl_outlet_struct_> handle() { return obj; }

    /** Destructor.
     * The stream will no longer be discoverable after destruction and all
     * paired inlets will stop delivering data.
     */
  ...
  private:
    ...
    /// Check whether a given data length matches the number of channels; throw
    /// if not
    void check_numchan(std::size_t N) const {
        if (N != static_cast<std::size_t>(channel_count))
            throw std::runtime_error(
                "Provided element count (" + std::to_string(N) +
 ...
 * network. The network is usually the subnet specified at the local router, but
 * may also include a multicast group of machines (given that the network
 * supports it), or list of hostnames. These details may optionally be
 * customized by the experimenter in a configuration file (see Network
 * Connectivity in the LSL wiki). This is the default mechanism used by the
 * browsing programs and the recording program.
 * @param wait_time The waiting time for the operation, in seconds, to search
 * for streams. If this is too short (<0.5s) only a subset (or none) of the
 * outlets that are present on the network may be returned.
 * @return A vector of stream info objects (excluding their desc field), any of
 * which can subsequently be used to open an inlet. The full info can be
 * retrieve from the inlet.
 */
inline std::vector<stream_info> resolve_streams(double wait_time = 1.0) {
    lsl_streaminfo buffer[1024];
    int nres = check_error(lsl_resolve_all(
        buffer, sizeof(buffer) / sizeof(lsl_streaminfo), wait_time));
 ...
 * @param prop The stream_info property that should have a specific value (e.g.,
 * "name", "type", "source_id", or "desc/manufaturer").
 * @param value The string value that the property should have (e.g., "EEG" as
 * the type property).
 * @param minimum Return at least this number of streams.
 * @param timeout Optionally a timeout of the operation, in seconds (default: no
 * timeout). If the timeout expires, less than the desired number of streams
 * (possibly none) will be returned.
 * @return A vector of matching stream info objects (excluding their meta-data),
 * any of which can subsequently be used to open an inlet.
 */
inline std::vector<stream_info> resolve_stream(const std::string& prop,
    ...
    int nres = check_error(
        lsl_resolve_byprop(buffer, sizeof(buffer) / sizeof(lsl_streaminfo),
                           prop.c_str(), value.c_str(), minimum, timeout));
    return std::vector<stream_info>(&buffer[0], &buffer[nres]);
}
 ...
 * [XPath 1.0](http://en.wikipedia.org/w/index.php?title=XPath_1.0) predicate
 ...
 * any of which can subsequently be used to open an inlet.
 */
inline std::vector<stream_info> resolve_stream(const std::string& pred,
                                               int32_t minimum = 1,
                                               double timeout = FOREVER) {
    lsl_streaminfo buffer[1024];
    int nres = check_error(
        lsl_resolve_bypred(buffer, sizeof(buffer) / sizeof(lsl_streaminfo),
                           pred.c_str(), minimum, timeout));
    return std::vector<stream_info>(&buffer[0], &buffer[nres]);
}
...
class stream_inlet {
  public:
     ...
     * @param info A resolved stream info object (as coming from one of the
     * resolver functions). Note: The stream_inlet may also be constructed with
     * a fully-specified stream_info, if the desired channel format and count is
     * already known up-front, but this is strongly discouraged and should only
     * ever be done if there is no time to resolve the stream up-front (e.g.,
     * due to limitations in the client program).
     * @param max_buflen Optionally the maximum amount of data to buffer (in
     * seconds if there is a nominal sampling rate, otherwise x100 in samples).
     ...
     * their next calculation.
     * @param max_chunklen Optionally the maximum size, in samples, at which
     * chunks are transmitted (the default corresponds to the chunk sizes used
     * by the sender). Recording applications can use a generous size here
     * (leaving it to the network how to pack things), while real-time
     * applications may want a finer (perhaps 1-sample) granularity. If left
     * unspecified (=0), the sender determines the chunk granularity.
     * @param recover Try to silently recover lost streams that are recoverable
     * (=those that that have a source_id set). In all other cases (recover is
     * false or the stream is not recoverable) functions may throw a
     * lsl::lost_error if the stream's source is lost (e.g., due to an app or
     * computer crash).
     */
    stream_inlet(const stream_info& info, int32_t max_buflen = 360,
                 int32_t max_chunklen = 0, bool recover = true,
                 lsl_transport_options_t flags = transp_default)
        : channel_count(info.channel_count()),
          obj(lsl_create_inlet_ex(info.handle().get(), max_buflen, max_chunklen,
                                  recover, flags),
              &lsl_destroy_inlet) {}
    ...
    /// yet
    ///
    /// Example: @code lsl_pull_sample_buf(inlet.handle().get(), buf, …);
    /// @endcode
     ...
     * @throws timeout_error (if the timeout expires), or lost_error (if the
     ...
     * transmission and buffering of data for this inlet will be stopped. If an
     * application stops being interested in data from a source (temporarily or
     * not) but keeps the outlet alive, it should call close_stream() to not
     ...
     * (and rely on periodic background updates). On a well-behaved network, the
     * precision of these estimates should be below 1 ms (empirically it is
     * within +/-0.2 ms).
     *
     * To get a measure of whether the network is well-behaved, use the extended
     * version time_correction(double*,double*,double) and check uncertainty
     * (i.e. the round-trip-time).
     *
     * 0.2 ms is typical of wired networks. 2 ms is typical of wireless
     * networks. The number can be much higher on poor networks.
     *
     * @param timeout Timeout to acquire the first time-correction estimate
     * (default: no timeout).
     * @return The time correction estimate. This is the number that needs to be
     * added to a time stamp that was remotely generated via lsl_local_clock()
     * to map it into the local clock domain of this machine.
     * @throws #lsl::timeout_error (if the timeout expires), or #lsl::lost_error
     ...
     * @param remote_time The current time of the remote computer that was used
     * to generate this time_correction. If desired, the client can fit
     * time_correction vs remote_time to improve the real-time time_correction
     * further.
     * @param uncertainty The maximum uncertainty of the given time correction.
     */
    double time_correction(double* remote_time, double* uncertainty,
                           double timeout = FOREVER) {
        int32_t ec = 0;
        double res = lsl_time_correction_ex(obj.get(), remote_time, uncertainty,
                                            timeout, &ec);
        check_error(ec);
        return res;
    }
     ...
     *
     * By default, the inlet performs NO post-processing and returns the
     * ground-truth time stamps, which can then be manually synchronized using
     * .time_correction(), and then smoothed/dejittered if desired.<br> This
     * function allows automating these two and possibly more operations.<br>
     * @warning When you enable this, you will no longer receive or be able to
     * recover the original time stamps.
     * @param flags An integer that is the result of bitwise OR'ing one or more
     * options from processing_options_t together (e.g.,
     * `post_clocksync|post_dejitter`); the default is to enable all options.
     */
     ...
     * @param timeout The timeout for this operation, if any.  Use 0.0 to make
     * the function non-blocking.
     * @return The capture time of the sample on the remote machine, or 0.0 if
     * no new sample was available. To remap this time stamp to the local clock,
     * add the value returned by .time_correction() to it.
     ...
     * Handles type checking & conversion and allocates the necessary memory in
    ...
    double pull_sample(std::vector<std::string>& sample,
                       double timeout = FOREVER) {
        sample.resize(channel_count);
        return pull_sample(&sample[0], (int32_t)sample.size(), timeout);
    }
     ...
     * @throws lost_error (if the stream source has been lost).
     */
    double pull_sample(float* buffer, int32_t buffer_elements,
                       double timeout = FOREVER) {
        int32_t ec = 0;
        double res =
            lsl_pull_sample_f(obj.get(), buffer, buffer_elements, timeout, &ec);
        check_error(ec);
        return res;
    }
    double pull_sample(double* buffer, int32_t buffer_elements,
                       double timeout = FOREVER) {
        int32_t ec = 0;
        double res =
            lsl_pull_sample_d(obj.get(), buffer, buffer_elements, timeout, &ec);
        check_error(ec);
        return res;
    }
    double pull_sample(int64_t* buffer, int32_t buffer_elements,
                       double timeout = FOREVER) {
        int32_t ec = 0;
        double res =
            lsl_pull_sample_l(obj.get(), buffer, buffer_elements, timeout, &ec);
        check_error(ec);
        return res;
    }
    double pull_sample(int32_t* buffer, int32_t buffer_elements,
                       double timeout = FOREVER) {
        int32_t ec = 0;
        double res =
            lsl_pull_sample_i(obj.get(), buffer, buffer_elements, timeout, &ec);
        check_error(ec);
        return res;
    }
    double pull_sample(int16_t* buffer, int32_t buffer_elements,
                       double timeout = FOREVER) {
        int32_t ec = 0;
        double res =
            lsl_pull_sample_s(obj.get(), buffer, buffer_elements, timeout, &ec);
        check_error(ec);
        return res;
    }
    double pull_sample(char* buffer, int32_t buffer_elements,
                       double timeout = FOREVER) {
        int32_t ec = 0;
        double res =
            lsl_pull_sample_c(obj.get(), buffer, buffer_elements, timeout, &ec);
        check_error(ec);
        return res;
    }
    double pull_sample(std::string* buffer, int32_t buffer_elements,
                       double timeout = FOREVER) {
        int32_t ec = 0;
        if (buffer_elements) {
            std::vector<char*> result_strings(buffer_elements);
            std::vector<uint32_t> result_lengths(buffer_elements);
            double res = lsl_pull_sample_buf(obj.get(), result_strings.data(),
                                             result_lengths.data(),
                                             buffer_elements, timeout, &ec);
            check_error(ec);
            ...
            throw std::runtime_error("Provided element count does not match "
     ...
     * @param sample The raw sample object to hold the data (packed C-style
     * struct). Search for [`#``pragma
     * pack`](https://stackoverflow.com/a/3318475/73299) for information on how
     * to pack structs correctly.
     * @param timeout The timeout for this operation, if any. Use 0.0 to make
     * the function non-blocking.
     * @return The capture time of the sample on the remote machine, or 0.0 if
     * no new sample was available. To remap this time stamp to the local clock,
     ...
     * @throws lost_error (if the stream source has been lost).
     */
    double pull_numeric_raw(void* sample, int32_t buffer_bytes,
                            double timeout = FOREVER) {
        int32_t ec = 0;
        double res =
            lsl_pull_sample_v(obj.get(), sample, buffer_bytes, timeout, &ec);
        check_error(ec);
        return res;
    }
     ...
     *
     * This is the most complete version, returning both the data and a
     ...
     */
    template <class T>
    bool pull_chunk(std::vector<std::vector<T>>& chunk,
     ...
     * Pull a chunk of samples from the inlet.
     *
     * This function does not return time stamps for the samples. Invoked as:
     * mychunk = pull_chunk<float>();
     * @return A vector of vectors containing the obtained samples; may be
     * empty.
     * @throws lost_error (if the stream source has been lost)
     */
     ...
     * @warning The provided buffer size is measured in channel values (e.g.,
     * floats), not samples.
     * @param data_buffer A pointer to a buffer of data values where the results
     * shall be stored.
     * @param timestamp_buffer A pointer to a buffer of timestamp values where
     * time stamps shall be stored. If this is NULL, no time stamps will be
     * returned.
     * @param data_buffer_elements The size of the data buffer, in channel data
     * elements (of type T). Must be a multiple of the stream's channel count.
     * @param timestamp_buffer_elements The size of the timestamp buffer. If a
     * timestamp buffer is provided then this must correspond to the same number
     * of samples as data_buffer_elements.
     * @param timeout The timeout for this operation, if any. When the timeout
     * expires, the function may return before the entire buffer is filled. The
     * default value of 0.0 will retrieve only data available for immediate
     * pickup.
     * @return data_elements_written Number of channel data elements written to
     * the data buffer.
     * @throws lost_error (if the stream source has been lost).
     */
    std::size_t pull_chunk_multiplexed(float* data_buffer,
                                       double* timestamp_buffer,
                                       std::size_t data_buffer_elements,
                                       std::size_t timestamp_buffer_elements,
                                       double timeout = 0.0) {
        int32_t ec = 0;
        std::size_t res = lsl_pull_chunk_f(
            obj.get(), data_buffer, timestamp_buffer,
            (unsigned long)data_buffer_elements,
            (unsigned long)timestamp_buffer_elements, timeout, &ec);
        check_error(ec);
        return res;
    }
    std::size_t pull_chunk_multiplexed(double* data_buffer,
                                       double* timestamp_buffer,
                                       std::size_t data_buffer_elements,
                                       std::size_t timestamp_buffer_elements,
                                       double timeout = 0.0) {
        int32_t ec = 0;
        std::size_t res = lsl_pull_chunk_d(
            obj.get(), data_buffer, timestamp_buffer,
            (unsigned long)data_buffer_elements,
            (unsigned long)timestamp_buffer_elements, timeout, &ec);
        check_error(ec);
    ...
    std::size_t pull_chunk_multiplexed(int64_t* data_buffer,
                                       double* timestamp_buffer,
                                       std::size_t data_buffer_elements,
                                       std::size_t timestamp_buffer_elements,
                                       double timeout = 0.0) {
        int32_t ec = 0;
        std::size_t res = lsl_pull_chunk_l(
            obj.get(), data_buffer, timestamp_buffer,
            (unsigned long)data_buffer_elements,
            (unsigned long)timestamp_buffer_elements, timeout, &ec);
        check_error(ec);
    ...
    std::size_t pull_chunk_multiplexed(int32_t* data_buffer,
                                       double* timestamp_buffer,
                                       std::size_t data_buffer_elements,
                                       std::size_t timestamp_buffer_elements,
                                       double timeout = 0.0) {
        int32_t ec = 0;
        std::size_t res = lsl_pull_chunk_i(
            obj.get(), data_buffer, timestamp_buffer,
            (unsigned long)data_buffer_elements,
            (unsigned long)timestamp_buffer_elements, timeout, &ec);
        check_error(ec);
    ...
    std::size_t pull_chunk_multiplexed(int16_t* data_buffer,
                                       ...
                                       double timeout = 0.0) {
        int32_t ec = 0;
        std::size_t res = lsl_pull_chunk_s(
            obj.get(), data_buffer, timestamp_buffer,
            (unsigned long)data_buffer_elements,
            (unsigned long)timestamp_buffer_elements, timeout, &ec);
        check_error(ec);
    ...
    std::size_t pull_chunk_multiplexed(char* data_buffer,
                                       double* timestamp_buffer,
                                       std::size_t data_buffer_elements,
                                       std::size_t timestamp_buffer_elements,
                                       double timeout = 0.0) {
        int32_t ec = 0;
        std::size_t res = lsl_pull_chunk_c(
            obj.get(), data_buffer, timestamp_buffer,
            static_cast<unsigned long>(data_buffer_elements),
            static_cast<unsigned long>(timestamp_buffer_elements), timeout,
            &ec);
        check_error(ec);
        return res;
    }
    std::size_t pull_chunk_multiplexed(std::string* data_buffer,
                                       double* timestamp_buffer,
                                       std::size_t data_buffer_elements,
                                       std::size_t timestamp_buffer_elements,
                                       double timeout = 0.0) {
        ...
        if (data_buffer_elements) {
            ...
            std::size_t num = lsl_pull_chunk_buf(
                obj.get(), result_strings.data(), result_lengths.data(),
                timestamp_buffer,
                static_cast<unsigned long>(data_buffer_elements),
                static_cast<unsigned long>(timestamp_buffer_elements), timeout,
                &ec);
    ...
    /**
     * Pull a multiplexed chunk of samples and optionally the sample timestamps
     * from the inlet.
     *
     ...
     * @param timeout Time to wait for the first sample. The default value of
     * 0.0 will not wait for data to arrive, pulling only samples already
     * received.
    ...
    bool pull_chunk_multiplexed(std::vector<T>& chunk,
        ...
        std::vector<T> sample;
        double ts;
        if ((ts = pull_sample(sample, timeout)) == 0.0)
            return false;
     ...
     */
    template <class T>
    bool pull_chunk_numeric_structs(std::vector<T>& chunk,
                                    std::vector<double>& timestamps) {
        T sample;
        chunk.clear();
        timestamps.clear();
     ...
     * Pull a chunk of samples from the inlet.
     *
     * This function does not return time stamps. Invoked as: mychunk =
     * pull_chunk<mystruct>();
     * @return A vector of C-style structs containing the obtained samples; may
     * be empty.
     * @throws lost_error (if the stream source has been lost)
     */
     ...
     * whether a pull_*() call would block: to be sure, set the pull timeout to
     * 0.0 or an acceptably low value. If the underlying implementation supports
     * it, the value will be the number of samples available (otherwise it will
     * be 1 or 0).
     */
     ...
     * was_clock_reset().
     *
     * This is a rarely-used function that is only useful to applications that
     * combine multiple time_correction values to estimate precise clock drift;
     * it allows to tolerate cases where the source machine was hot-swapped or
     * restarted in between two measurements.
     */
     ...
     * file. Using a longer window will yield lower jitter in the time stamps,
     ...
     * changes up to 10 degrees C per minute sufficiently well.
     */
    void smoothing_halftime(float value) {
        check_error(lsl_smoothing_halftime(obj.get(), value));
    }
 ...
 * value; attributes are omitted. Insider note: The interface is modeled after a
 * subset of pugixml's node type and is compatible with it. See also
...
class xml_element {
  public:
    ...
    xml_element child(const std::string& name) const {
        return lsl_child(obj, (name.c_str()));
    }
    ...
    /// Is this a text body (instead of an XML element)? True both for plain
    /// char data and CData.
    ...
    xml_element prepend_child_value(const std::string& name,
                                    const std::string& value) {
    ...
    bool set_name(const std::string& rhs) {
        return lsl_set_name(obj, rhs.c_str()) != 0;
    }
    ...
    bool set_value(const std::string& rhs) {
        return lsl_set_value(obj, rhs.c_str()) != 0;
    }
    ...
    xml_element append_copy(const xml_element& e) {
        return lsl_append_copy(obj, e.obj);
    }

    /// Prepend a child element with the specified name.
    xml_element prepend_copy(const xml_element& e) {
        return lsl_prepend_copy(obj, e.obj);
    }

    /// Remove a child element with the specified name.
    void remove_child(const std::string& name) {
        lsl_remove_child_n(obj, (name.c_str()));
    }
...
class continuous_resolver {
  public:
     ...
     * resolve_streams().
     * @param forget_after When a stream is no longer visible on the network
     * (e.g., because it was shut down), this is the time in seconds after which
     * it is no longer reported by the resolver.
     */
    ...
    /**
     * Construct a new continuous_resolver that resolves all streams with a
     * specific value for a given property.
     *
     * This is analogous to the functionality provided by the free function
     * resolve_stream(prop,value).
     * @param prop The stream_info property that should have a specific value
     * (e.g., "name", "type", "source_id", or "desc/manufaturer").
     * @param value The string value that the property should have (e.g., "EEG"
     * as the type property).
     * @param forget_after When a stream is no longer visible on the network
     * (e.g., because it was shut down), this is the time in seconds after which
     * it is no longer reported by the resolver.
     */
    continuous_resolver(const std::string& prop, const std::string& value,
    ...
    /**
     * Construct a new continuous_resolver that resolves all streams that match
     * a given XPath 1.0 predicate.
     *
     * This is analogous to the functionality provided by the free function
     * resolve_stream(pred).
     ...
     * count(info/desc/channel)=32`
     * @param forget_after When a stream is no longer visible on the network
     * (e.g., because it was shut down), this is the time in seconds after which
     * it is no longer reported by the resolver.
     */
    continuous_resolver(const std::string& pred, double forget_after = 5.0)
        : obj(lsl_create_continuous_resolver_bypred((pred.c_str()),
                                                    forget_after),
              &lsl_destroy_continuous_resolver) {}

    /**
     * Obtain the set of currently present streams on the network (i.e. resolve
     * result).
     * @return A vector of matching stream info objects (excluding their
     * meta-data), any of which can subsequently be used to open an inlet.
     */
    std::vector<stream_info> results() {
        lsl_streaminfo buffer[1024];
        return std::vector<stream_info>(
            buffer, buffer + check_error(lsl_resolver_results(obj.get(), buffer,
                                                              sizeof(buffer))));
    }
    ...
    continuous_resolver&
    operator=(continuous_resolver&& rhs) noexcept = default;

  private:
    std::unique_ptr<lsl_continuous_resolver_,
                    void (*)(lsl_continuous_resolver_*)>
        obj;
};
...
/// Exception class that indicates that a stream inlet's source has been
/// irrecoverably lost.
...
/// Check error codes returned from the C interface and translate into
...
inline int32_t check_error(int32_t ec) {
    if (ec < 0) {
        ...
        case lsl_lost_error:
            throw lost_error("The stream has been lost; to continue reading, "
                             "you need to re-resolve it.");
